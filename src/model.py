...
class GitHubClassifier:
    ...
    def classify(self,X):
        ...
        df = DataFrame(np.concatenate((X.reshape(-1,1), y.reshape(-1, 1)), axis=1))
        df.columns = ['x', 'y']
        pd_train = pd.read_csv("final_data.csv")
        columns = list(pd_train.columns)
        dict = {}
        for col in columns:
            if col != 'Unnamed: 0':
                dict[col] = np.zeros(len(X))
        x_to_pred, temp = create_features(df, dict)
        # load the model and predict
        loaded_model = pickle.load(open('finalized_model.sav', 'rb'))
        return loaded_model.predict(x_to_pred)
